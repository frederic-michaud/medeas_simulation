...
for L in Ls:
    for D in Ds:
        eigenvalue_summary_file = os.path.join(result_subfolder_fullpath, f'L_{L}_D_{D}.dat')
        summary_file = open(eigenvalue_summary_file, "wb")
        all_eg = []
        for _ in range(sample_size):
            ...
            eigenvalue_file = os.path.join(output_folder, "MDS_eigensystem","p1.vecs.data")
            with open(eigenvalue_file,"rb") as f:
                lambdap1, _ = pickle.load(f)
            eigenvalue_file = os.path.join(output_folder, "MDS_eigensystem", "p2.vecs.data")
            with open(eigenvalue_file,"rb") as f:
                lambdap2, _ = pickle.load(f)
            all_eg.append((lambdap1,lambdap2))

        pickle.dump(all_eg,summary_file)
        summary_file.close()
