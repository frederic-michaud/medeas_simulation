...
def run_simulation_two_pops(n1: int, n2: int, L: int, theta: float, D: float, Ne_small, output_folder):
    ...
    scrm_command = f'{n1+n2} {L} -t {theta} -I 2 {n1} {n2} -ej {D} 2 1 -en 0 2 {Ne_small} --print-model -l -1 -L'
...
for L in Ls:
    for D in Ds:
        for Ne in Nes:
            distance_summary_file = os.path.join(result_subfolder_fullpath, f'L_{L}_D_{D}_Ne_{Ne}.dat')
            summary_file = open(distance_summary_file, "w")
            coalescence_summary_file = os.path.join(result_subfolder_fullpath, f'L_{L}_D_{D}_Ne_{Ne}_coal.dat')
            coalescence_summary = open(coalescence_summary_file, "w")
            for _ in range(sample_size):
                simulation_subsubfolder = f'L_{L}_D_{D}_Ne_{Ne}'
                ...
                n2 = 20
                theta = 2.
                run_simulation_two_pops(n1, n2, int(L), theta, D, Ne, output_folder)
