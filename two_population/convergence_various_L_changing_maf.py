...
def run_simulation_two_pops(n1: int, n2: int, L: int, theta: float, D: float,output_folder: str, maf: float):
    ...
    transcode_commande = f'python {location_transcode} {scrm_result} {snip_file} {n1+n2} {maf}'
...
Ls = [int(10**(i/4)) for i in range(12, 25)] #regulary space with 4 point between each order of magnitude
mafs = [0.05]
Ds = [0.01]
sample_size = 100
current_folder = os.path.dirname(os.path.realpath(__file__))
...
for L in Ls:
    for D in Ds:
        for maf in mafs:
            distance_summary_file = os.path.join(result_subfolder_fullpath, f'L_{L}_D_{D}_maf_{maf}.dat')
            summary_file = open(distance_summary_file, "w")
            for _ in range(sample_size):
                simulation_subsubfolder = f'L_{L}_D_{D}_maf_{maf}'
                output_folder = os.path.join(current_folder, simulation_subfolder, simulation_subsubfolder)
                if not os.path.exists(output_folder):
                    os.makedirs(output_folder)
                n1 = 100
                n2 = 100
                run_simulation_two_pops(n1, n2, int(L), 2, D,output_folder,maf)
                ...
                if os.path.isfile(distance_file):
                    ...
                    distance = np.mean(distances[distances>0])
                    sd = np.std(distances[distances>0])
