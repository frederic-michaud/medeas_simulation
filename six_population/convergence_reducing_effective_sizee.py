...
# we then prograssively decrease the population sizes.
...
def run_simulation_four_pops(n: int, L: int, theta: float, output_folder: str):
...
-ej {D_europe} 4 3 \
-ej {D_asia} 6 5 \
-en {0} 3 {n_europe_large} \
-en {0} 2 {n_africa_small} \
-en {0} 5 {n_asia_large} \
-en {0} 4 {n_europe_small} \
-en {0} 6 {n_asia_small} \
--print-model -l -1 -L'
    ...
    command = " ".join(['python', location_run_medeas, snip_file, label_file, output_folder])
    print(command)
...
sample_size = 1
current_folder = os.path.dirname(os.path.realpath(__file__))
simulation_subfolder = "decreasing_effective_size"
if not os.path.exists(simulation_subfolder):
    os.mkdir(simulation_subfolder)
for L in Ls:
    distance_summary_file = os.path.join(simulation_subfolder,f'L_{L}.between')
    summary_file = open(distance_summary_file, "w")
    within_summary_file = os.path.join(simulation_subfolder,f'L_{L}.within')
    ...
    for _ in range(sample_size):
        ...
        run_simulation_four_pops(n, L, theta, output_folder)
