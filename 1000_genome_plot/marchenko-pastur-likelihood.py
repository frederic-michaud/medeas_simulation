...
def log_likelihood(xs, T, L, N):
    likelihood = np.sum(np.log([density_function(x, T, L, N) for x in xs]))
    return likelihood

#%%
def get_T_and_L_mle(lambdas):
    N = len(lambdas)

    def get_range_T_L():
        lambda_max = 1.01*lambdas[-1]
        lambda_min = lambdas[1]/1.01
    ...
    def log_likelihood_given(x):
        return -log_likelihood(lambdas, x[0], x[1], N)
    T, L = scipy.optimize.fmin(log_likelihood_given,(T_initial,L_initial))
    print((T,L))
    return((T, L))
...
def CDF_MP_not_normlized(x: float,a: float, b: float) -> float:
    ...
    return res
def CDF_MP(x: float,a: float, b: float) -> float:
    """Integral of Marchenko-Pastur distribution function on interval
    a < x < b.
    """
...
def get_pp_points(lambdas):
    ...
    N = len(lambdas)
    a, b = get_density_bound(T, L, N)
    tobeplotted = [CDF_MP(x, a, b) for x in lambdas]
...
fig.text(0.5, 0.04, 'Eigenvalue', ha='center')
fig.text(0.04, 0.5, 'Eigenvalue density', va='center', rotation='vertical')
plt.savefig("marchenko-pastur_likelihood.pdf")
...
plt.legend()
plt.savefig("pp_plot_marchenko_pastur_likelihood.pdf")


#%% Plotting individuals plot
plt.show()
plt.cla()
simulation_subfolder = "../../mds_1000_genome/random_prunning/medeas/single/YRI/"
val, vec = pickle.load(open(os.path.join(simulation_subfolder, "MDS_eigensystem/p2.vecs.data"), "rb"))
plot_mp_vs_histogram(val[1:], show=False, title="mp_random_prunning.pdf")

#%%
simulation_subfolder = "../../mds_1000_genome/plink_prunning/medeas/single/YRI/"
val, vec = pickle.load(open(os.path.join(simulation_subfolder, "MDS_eigensystem/p2.vecs.data"), "rb"))
plot_mp_vs_histogram(val[1:], show=False, title="mp_plink_prunning.pdf")

#%%
simulation_subfolder = "../../mds_1000_genome/no_prunning/medeas/single/YRI/"
val, vec = pickle.load(open(os.path.join(simulation_subfolder, "MDS_eigensystem/p2.vecs.data"), "rb"))
plot_mp_vs_histogram(val[1:], show=False, title="no_prunning.pdf")
...
simulation_subfolder = "single_population/marchenko_pastur_cut_maf"
simulation_subsub_folder = os.path.join(simulation_subfolder,f"L_10000")
val, vec = pickle.load(open(os.path.join(simulation_subsub_folder, "MDS_eigensystem/p2.vecs.data"), "rb"))
plot_mp_vs_histogram(val[1:], show=False, title="no_prunning_cut_maf.pdf")
...
simulation_subsub_folder = os.path.join(simulation_subfolder,f"L_10000")
val, vec = pickle.load(open(os.path.join(simulation_subsub_folder, "MDS_eigensystem/p2.vecs.data"), "rb"))
pp_simulation_cut_maf = get_pp_points(val[1:])
plt.plot(pp_simulation, np.linspace(0,1,num= len(pp_simulation)),label = "Simulated value")
plt.plot(pp_random_prunning, np.linspace(0,1,num= len(pp_random_prunning)), label = "Random prunning")
plt.plot(pp_plink_prunning, np.linspace(0,1,num= len(pp_plink_prunning)), label = "Plink prunning")
plt.plot(pp_no_prunning, np.linspace(0,1,num= len(pp_no_prunning)), label = " No prunning")
plt.plot(pp_simulation_cut_maf, np.linspace(0,1,num= len(pp_simulation_cut_maf)), label = "simulation cut maf")
plt.plot([0,1],[0,1],'k',lw = 0.5, label = "expected")
plt.xlabel("Theoretical distribution")
plt.ylabel("Observed distribution")
plt.legend()
plt.show()
#plt.savefig("pp_plot_marchenko_pastur.pdf")
