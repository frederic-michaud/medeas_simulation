...
def run_simulation_n_pops(n_pop : int,n_individual_per_pop: int, L: int,
    ...
    bootstrapnumber = 10
    command = " ".join(['python',location_run_medeas, snip_file,label_file,output_folder,str(bootwindow), str(bootstrapnumber)])
    print(command)
...
D = 0.05
current_folder = os.path.dirname(os.path.realpath(__file__))
simulation_subfolder = os.path.join(current_folder, "convergence_speed")
    ...
    for K in Ks:
        ...
        for _ in range(sample_size):
            ...
            if os.path.isfile(distance_file):
                distances = np.loadtxt(distance_file, ndmin=2, skiprows=1)
                distances_average = np.mean(distances, axis=0)
                summary_file.write(" ".join(map(str, distances_average)) + '\n')
                summary_file.flush()
            else:
                warnings.warn(f'the file {distance_file} does not exist')
